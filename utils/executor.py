...
class Executor(Commoner):
    prompt = """你是一个有用的助手。请你根据用户问题、问题规划和历史执行信息，执行所要求的步骤。请一步一步执行，不要多执行。
...
{action}"""
    ...
    def predict(self, plan, action, analysis="", history=""):
        self.action_output = ""
...
4. **步骤4：最终确认**
   - 在限定的选择中找到只有一张符合所有信息描述条件的答案。"""
    ...
    executor = Executor(question=question)
    for token in executor.predict(analysis=analysis, plan=plan, action=action, history=""):
