...
class Summarizer(_Commoner):
    system_prompt = """你是一个帮助大语言模型凝练、总结最后答案的助手。"""
    user_prompt = """## 用户问题：
```text
{question}
```
...
请你根据以上信息，凝练、总结能够解答用户问题的、正确的最终答案。
"""
...
接下来步骤2和后续分析中，请严格遵循逻辑关系来进行筛选，结合对话中的关键信息逐一排除可能性直到锁定正确答案。这样的改动能更好与原问题及解析对应，并保持逻辑连贯且严谨。"""
    ...
    for token in summarizer.predict(question=question, analysis=analysis, plan=plan, history="", action=action,
                                    action_output=action_output, criticism=criticism):
        print(token, end='', flush=True)
    print('\n\n\n')
    print(f"summarizer.get_summary : {summarizer.get_summary()}")
