...
class Criticor(_Commoner):
    system_prompt = """你是一个逻辑批评家。你需要根据用户问题和历史执行信息，对问题规划和当前执行信息进行批评。请注意：整个问题规划是一步一步执行的，所以批评理由不要是还没执行某个步骤；如果当前信息没有问题，也请给出理由。"""
    user_prompt = """## 用户问题：
```text
{question}
```
...
请输出批评理由:"""
    ...
    def predict(self, question, plan, action, action_output, analysis="", history=""):
        self.criticism = ""
...
接下来，我们将按照提供的对话，逐一分析这16张牌的信息，并最终确定那张唯一的牌。"""
    ...
    criticor = Criticor()
    for token in criticor.predict(question=question, analysis=analysis, plan=plan, action=action,
                                  action_output=action_output, history=""):
